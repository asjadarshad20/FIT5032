...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(day, type) in bookstores.openingHours" :key="type">
          {{ type }}: {{ day.open }}
        </li>
      </ul>
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="countries in bookstores.countries" :key="countries">{{ countries }}</li>
      </ul>

      <p>Our #1 sellers:</p>
      <ul>
        <li v-for="countries in bookstores.topSellers" :key="countries">{{ countries }}</li>
      </ul>
    </section>
...
const orwell = computed(() => {
  return authors.filter((authors) => authors.name == 'George Orwell')
})

// Activity 5: Find author by ID
const austen = computed(() => {
  return authors.filter((authors) => authors.id == 1)
})
</script>
